...
export async function sendOrderConfirmation(data: OrderEmailData) {
  ...
  const itemsHtml = items
    ...
    <tr>
      <td style="padding: 12px; border-bottom: 1px solid #e2e2e2;">
        ${item.name}
        ${item.size || item.color ? `<br><small style="color: #777;">${[item.size, item.color].filter(Boolean).join(", ")}</small>` : ""}
      </td>
      ...
      <td style="padding: 12px; border-bottom: 1px solid #e2e2e2; text-align: right;">
        ${((item.priceCents * item.quantity) / 100).toLocaleString("ro-RO", { style: "currency", currency: "RON" })}
      </td>
    </tr>
  `
    )
  ...
  try {
    const result = await resend.emails.send({
      ...
      to: email,
      subject: `Confirmare Comandă #${orderId.slice(0, 8)}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: Inter, Arial, sans-serif; background-color: #f9f9f9; margin: 0; padding: 0;">
          <div style="max-width: 600px; margin: 0 auto; padding: 40px 24px;">
            <h1 style="font-family: 'Noto Serif', Georgia, serif; font-size: 32px; margin-bottom: 8px;">RED STUDIO</h1>
            <p style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.2em; color: #5f5e5e; margin-bottom: 40px;">
              Confirmare Comandă
            </p>
            
            <p style="font-size: 16px; line-height: 1.6; margin-bottom: 24px;">
              Mulțumim${fullName ? `, ${fullName}` : ""} pentru comanda dumneavoastră.
            </p>
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
              <thead>
                <tr style="border-bottom: 2px solid #1a1c1c;">
                  ...
                  <th style="padding: 12px; text-align: right; font-size: 10px; text-transform: uppercase; letter-spacing: 0.2em;">Preț</th>
                </tr>
              </thead>
              <tbody>
                ${itemsHtml}
            ...
            <p style="font-size: 10px; text-transform: uppercase; letter-spacing: 0.1em; color: #777; text-align: center;">
              © 2024 RED STUDIO. Toate drepturile rezervate.
            </p>
          </div>
        </body>
        </html>
